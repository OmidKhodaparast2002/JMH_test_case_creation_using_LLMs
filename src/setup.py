...
def check_model_connectivity(model_url: str, model_name: str) -> bool:
    ...
    try:
        ...
        response = requests.post(
            url, json={"model": model_name, "prompt": "Hello", "stream": False}
        )
...
def clone_projects(project_data: dict, dest_folder: Path):
    ...
    try:
        for url in project_urls:
            ...
            repo_name = Path(
                re.search(r"/([^/]+?)(?:\.git)?$", url).group(1)
            )  # regex to get repo name
...
def prep_projects_modules(project_data: dict) -> dict:
    ...
    for project in project_data["projects"]:
        ...
        if os.path.isdir(analysis_path):
            ...
            ]  # stringify it for being a serializable json

        else:
            print(
                f"Given analysis path is not a directory for given project: {project['name']}"
            )
